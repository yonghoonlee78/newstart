describe('Array에 대해서 학습합니다.', function () {
  it('Array의 기본을 확인합니다.', function () {
    ...
    expect(multiTypeArr.length).to.equal(?);
    ...
    expect(multiTypeArr[?][?]).to.equal(5);
    expect(multiTypeArr[?][?]).to.equal(7);
  });
  ...
  it('Array 메소드 slice를 확인합니다.', function () {
    ...
    expect(arr.slice(1)).to.deep.equal();
  ...
  it('Array를 함수의 인자로 전달할 경우, reference가 전달됩니다.', function () {
    ...
    expect(arr[5]).to.equal('changed in function');
  ...
  it('Array 메소드 shift와 unshift를 확인합니다.', function () {
    ...
    expect(shiftedValue).to.deep.equal(?);
    expect(arr).to.deep.equal([?);
  });
});
